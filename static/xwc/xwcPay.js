...
var createDeaultQRContainer = function (options) {
	...
	qrcontainer.onclick = function (event) {
...
XwcPay.prototype = {
	...
	defaultConfig: {
		...
		network: 'wss://node8.whitecoin.info:8877' // 'ws://nodeapi.xwclab.org:6090' // 'wss://nodeapi.xwclab.org'
	},
  ...
  waitTransactionWithAuction: function (nodeClient, txid, timeout) {
		timeout = timeout || 8000;
		return new Promise(function (resolve, reject) {
			var executedTimeout = 0;
			var lastError = "transaction timeout, maybe transaction not successfully";
			var intervalFunc = function () {
				...
				executedTimeout += 2000;
				nodeClient.getContractTxReceipt(txid).then(function (tx) {
					clearInterval(intervalHandler);
					resolve(tx);
				}).catch(function (err) {});
			};
