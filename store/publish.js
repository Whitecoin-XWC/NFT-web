...
export const actions = {
  ...
  async fileUpload_save({ commit }, payload) {
    const res = await http(this.$axios).fileUpload(payload)
    if (res.code === 200) {
      commit('set_state', { tokenId: res.data.tokenId })
      return res
    }
  },
}
